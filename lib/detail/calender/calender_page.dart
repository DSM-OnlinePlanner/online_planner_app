...
class CalenderPage extends StatefulWidget {
  String tier;
  CalenderPage(this.tier);

  _CalenderWidget createState() => _CalenderWidget(tier);
}

class _CalenderWidget extends State<CalenderPage> {
  _CalenderWidget(this.initTier);

  List<CalenderPlanner> calenderPlanners = [];
  late Future<List<CalenderPlanner>> calenders;
  String initTier = "";
  String nickName = "",
      tier = "";
  int exp = -1,
  ...
  Future<List<CalenderPlanner>> _getCalender() async {
    ...
    try {
      ...
      final response = await dio.get(
          ...
          options: Options(
              headers: {
                HttpHeaders.authorizationHeader: sharedPreferences.getString('accessToken'),
              }),
          queryParameters: {
      ...
      if(response.statusCode == 200) {
        calenderPlanners = (response.data as List).map((e) => CalenderPlanner.fromJson(e)).toList();
      }

      return calenderPlanners;
    }catch(e) {
      await _retryCalender();

      return calenderPlanners;
    }
  }

  _retryCalender() async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    var date = DateTime.now();
    var format = 'yyyy-MM-dd';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: PageView(
        ...
        children: [
          FutureBuilder <List<CalenderPlanner>>(
            future: calenders,
                ...
                return SingleChildScrollView(
                  child: Container(
                    child: SfCalendar(
                      ...
                      todayHighlightColor: Color(0xff2F5DFB),
                      ...
                      headerHeight: 40,
                      onLongPress: (d) {
                        print('touch : ' + initTier);
                        Navigator.pushAndRemoveUntil(
                            context, 
                            PageTransition(
                                child: PlannerPage(initTier, DateFormat('yyyy-MM-dd').format(d.date!)),
                                type: PageTransitionType.bottomToTop), 
                                (route) => false
                        );
                      },
                      dataSource: PlannerDataSource(snapshot.data!),
                    ),
                    width: _width,
                    height: _height * 0.8,
                  ),
...
class PlannerDataSource extends CalendarDataSource {
  PlannerDataSource(List<CalenderPlanner> source) {
    appointments = source;
  }

  @override
  DateTime getStartTime(int index) {
    return DateTime.parse((appointments![index] as CalenderPlanner).startDate);
  }

  @override
  DateTime getEndTime(int index) {
    return DateTime.parse((appointments![index] as CalenderPlanner).endDate);
  }

  @override
  Color getColor(int index) {
    String want = (appointments![index] as CalenderPlanner).want;

    if (want == "ONE")
      return Color(0xffFF4631);
    else if (want == "TWO")
      return Color(0xffFF974B);
    else if (want == "THREE")
      return Color(0xffFEBA2B);
    else if (want == "FOUR")
      return Color(0xff1BB778);
    else
      return Color(0xff2F5DFB);
  }

  @override
  String getSubject(int index) {
    return (appointments![index] as CalenderPlanner).title;
  }
}

class CalenderPlanner {
  String title, startDate, endDate, priority, want, startTime, endTime;
  bool isSucceed;

  CalenderPlanner({required this.title, required this.startDate, required this.endDate, required this.priority, required this.want, required this.isSucceed, required this.startTime, required this.endTime});

  factory CalenderPlanner.fromJson(Map<String, dynamic> json) {
    return CalenderPlanner(
      title: json['title'],
      startDate: json['startDate'],
      endDate: json['endDate'],
      priority: json['priority'],
      want: json['want'],
      isSucceed: json['succeed'],
